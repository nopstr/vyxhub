...
import { Zap, Mail, Lock, User, AtSign, Eye, EyeOff } from 'lucide-react'
    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-2xl shadow-white/10">
            ...
            {mode !== 'forgot' && (
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-[38px] text-zinc-500 hover:text-zinc-300 cursor-pointer"
                >
            ...
            {mode === 'signup' && (
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  ...
                  className="mt-1 w-4 h-4 rounded border-zinc-700 bg-zinc-800 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                />
                <span className="text-xs text-zinc-400 leading-relaxed">
                  I confirm that I am <strong className="text-white">18 years or older</strong> and agree to the{' '}
          ...
          <div className="mt-6 text-center space-y-2">
            {mode === 'login' && (
              <>
                ...
                <p className="text-sm text-zinc-500">
                  No account?{' '}
                  <button onClick={() => setMode('signup')} className="text-indigo-400 font-semibold hover:underline cursor-pointer">
                    Sign up
                  </button>
                </p>
              </>
            ...
            {mode === 'signup' && (
              <p className="text-sm text-zinc-500">
                ...
                <button onClick={() => setMode('login')} className="text-indigo-400 font-semibold hover:underline cursor-pointer">
                  Sign in
                </button>
              </p>
            )}
            {mode === 'forgot' && (
              <button onClick={() => setMode('login')} className="text-sm text-indigo-400 font-semibold hover:underline cursor-pointer">
                Back to login
              </button>
            )}
          </div>
