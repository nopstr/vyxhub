-- =============================================================================
-- Show ALL posts (including subscribers_only) in feeds to everyone.
-- The frontend handles lock state (blurred previews, PaywallGate, etc.)
-- This lets non-logged-in visitors see that premium content exists, enticing
-- them to sign up and subscribe.
-- =============================================================================

-- 1. Fix personalized_feed — return ALL posts regardless of visibility
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE
      -- All posts shown to everyone; frontend handles lock state (blurred preview / PaywallGate)
      (p.is_draft IS NULL OR p.is_draft = FALSE)
...
-- 2. Fix following_feed_ranked — return ALL posts
...
BEGIN
  ...
  WHERE
    -- All posts shown; frontend handles lock state
    (p.is_draft IS NULL OR p.is_draft = FALSE)
    AND NOT EXISTS (
      SELECT 1 FROM blocks
      WHERE (blocker_id = p_user_id AND blocked_id = p.author_id)
...
-- 3. Fix personalized_reels — return ALL reels
...
BEGIN
  ...
  WHERE p.post_type = 'video'
    -- All reels shown; frontend handles lock state
...
-- 4. Fix explore_posts — show ALL posts (not just public)
...
BEGIN
  ...
  LEFT JOIN velocity v ON v.pid = p.id
  WHERE
    -- All posts shown; frontend handles lock state for subscribers_only
    (p.is_draft IS NULL OR p.is_draft = FALSE)
