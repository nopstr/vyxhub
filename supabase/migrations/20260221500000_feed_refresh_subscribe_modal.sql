-- ============================================================================
-- FEED REFRESH FIX + SUBSCRIBE MODAL SUPPORT
-- February 21, 2026
-- ============================================================================
-- 1. Fix deterministic feed: add random jitter + progressive seen demotion
-- 2. Add subscription_benefits JSONB column to profiles for subscribe modal
-- ============================================================================


-- ============================================================================
-- 1. FIX: Add subscription_benefits to profiles
-- Stores an array of benefit strings set by the creator in settings.
-- Example: ["Exclusive behind-the-scenes", "Direct messages", "Weekly exclusives"]
-- ============================================================================
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS subscription_benefits JSONB DEFAULT '[]'::JSONB;


-- ============================================================================
-- 2. FIX: Deterministic feed — replace personalized_feed with random jitter
--    + progressive seen count demotion (not uniform 0.25x)
-- ============================================================================

-- First, add seen_count to post_impressions for progressive demotion
ALTER TABLE post_impressions ADD COLUMN IF NOT EXISTS seen_count INT DEFAULT 1;

-- Update record_post_impressions to increment seen_count on conflict
CREATE OR REPLACE FUNCTION record_post_impressions(
  p_user_id UUID,
  p_post_ids UUID[]
) RETURNS VOID AS $$
BEGIN
  INSERT INTO post_impressions (user_id, post_id, seen_count)
  SELECT p_user_id, unnest(p_post_ids), 1
...
-- ============================================================================
-- 3. Updated personalized_feed with:
--    a) Random jitter (±15%) so refresh gives variation
--    b) Progressive seen demotion: 1/(1+seen_count*0.3) instead of flat 0.25x
--    c) Both heuristic and ML paths get the jitter
-- ============================================================================
...
BEGIN
  ...
  candidates AS (
    SELECT
      ...
      -- Progressive seen tracking: seen_count instead of boolean
    ...
    SELECT
      ...
      (
        CASE WHEN v_use_ml THEN
          ...
          -- Progressive seen demotion: 1/(1 + seen_count * 0.3)
          * (1.0 / (1.0 + c.seen_count * 0.3))
          -- Random jitter ±15% for feed variation on refresh
          * (0.85 + random() * 0.30)
        ...
        ELSE
          ...
          (
            ...
            * CASE
                ...
                ELSE 0.5
              END
            -- Progressive seen demotion: 1/(1 + seen_count*0.3) instead of flat 0.25x
            * (1.0 / (1.0 + c.seen_count * 0.3))
...
-- NOTE: Changed from STABLE to VOLATILE because random() makes each call produce different results
