-- =============================================================================
-- FIX: Feed RPCs exclude creator's own subscriber-only posts
-- All feed RPCs had visibility filter:
--   p.visibility = 'public' OR (p.visibility = 'subscribers_only' AND subscription EXISTS)
-- Missing: OR p.author_id = p_user_id (creators should always see their own posts)
-- =============================================================================

-- 1. Fix personalized_feed — add author self-visibility
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE
      -- FIX: Creators can always see their own posts (including subscribers_only)
...
-- 2. Fix following_feed_ranked — add author self-visibility
...
BEGIN
  ...
  user_following AS (
    ...
    WHERE subscriber_id = p_user_id AND status = 'active' AND expires_at > NOW()
    UNION
    -- Include own posts in following feed (so creators see their own content)
    SELECT p_user_id
  )
  SELECT
    p.id, p.author_id, p.content, p.post_type, p.visibility, p.is_pinned,
  ...
  WHERE
    -- FIX: Creators can always see their own posts
    (p.author_id = p_user_id
      OR p.visibility = 'public'
...
-- 3. Fix personalized_reels — add author self-visibility
...
BEGIN
  ...
  WHERE p.post_type = 'video'
    AND (
      -- FIX: Creators can always see their own video posts
      p.author_id = p_user_id
      OR p.visibility = 'public'
      OR (
